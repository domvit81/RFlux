...
robf_despiking <- function(x, mfreq, wsignal = mfreq*30+1, wscale = mfreq*30+1, wby=1, zth=5){
	...
	if(wl <= mfreq*60+1){
		...
		spike_loc <- na.omit(as.vector(c(dspk[[1]]$spike_loc, dspk[[2]]$spike_loc + mfreq*60*5, dspk[[3]]$spike_loc + mfreq*60*5*2, dspk[[4]]$spike_loc + mfreq*60*5*3, dspk[[5]]$spike_loc + mfreq*60*5*4, dspk[[6]]$spike_lo + mfreq*60*5*5)));
